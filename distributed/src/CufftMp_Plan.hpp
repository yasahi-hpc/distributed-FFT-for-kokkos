...
template <typename ExecutionSpace, typename InViewType, typename OutViewType,
          std::size_t DIM>
bool is_tpl_available (const ExecutionSpace& exec_space, const InViewType& in,
  ...
  auto in_first_dim = in_topology.at(last_axis), out_first_dim = out_topology.at(last_axis);
...
std::size_t create_plan(const ExecutionSpace& exec_space,
  ...
  auto non_negative_axes =
        convert_negative_axes<std::size_t, int, 2, 2>(axes);
  auto fft_extents = get_fft_extents(gin_extents, gout_extents, non_negative_axes);  
  const auto [nx, ny] = fft_extents;
  std::size_t fft_size = std::accumulate(fft_extents.begin(), fft_extents.end(), 1,
                                 std::multiplies<std::size_t>());

  KOKKOSFFT_THROW_IF(in_topology == out_topology, "Input and output topologies must not be identical");
...
std::size_t create_plan(const ExecutionSpace& exec_space,
  ...
  // In case of slab geometry, we need to check that the first dimension is ready
  auto last_axis = axes.back();
  auto in_first_dim = in_topology.at(last_axis), out_first_dim = out_topology.at(last_axis);
  bool is_first_dim_ready = in_first_dim==1 && out_first_dim==1;
  bool is_slab =
  ...
  if (is_slab && is_first_dim_ready) {
    ...
    plan          = std::make_unique<PlanType>(nx, ny, nz, type, comm, is_xslab);
  } else {
    // PencilPlan is available
    bool is_pencil =
        is_pencil_topology(in_topology) && is_pencil_topology(out_topology);
    KOKKOSFFT_THROW_IF(!is_pencil,
                       "CufftMp only supports slab or pencil topologies for 3D transforms.");
    //plan          = std::make_unique<PlanType>(fft_extents, lower_input, upper_input,
    //                                           lower_output, upper_output, strides_input, strides_output,
    //                                            type, comm, is_xslab);
  }
