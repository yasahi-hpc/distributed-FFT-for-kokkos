...
inline void exec_plan(
    [[maybe_unused]] const ExecutionSpace& exec_space, const PlanType& scoped_plan,
    const InViewType& in, const OutViewType& out,
    [[maybe_unused]] const std::array<std::size_t, InViewType::rank()>& in_extents,
    [[maybe_unused]] const std::array<std::size_t, OutViewType::rank()>& out_extents,
  ...
  bool is_transpose_needed = KokkosFFT::Impl::is_transpose_needed(in_map) || KokkosFFT::Impl::is_transpose_needed(out_map);
  if (is_transpose_needed) {
    ...
    auto in_buffer = scoped_plan.template buffer_data<InRightViewType>(in_buffer_extents);
    auto in_T = InRightViewType(in.data(), KokkosFFT::Impl::create_layout<Kokkos::LayoutRight>(in_extents));

    Kokkos::deep_copy(exec_space, in_buffer, in);
    KokkosFFT::Impl::transpose(exec_space, in_buffer, in_T, in_map, true);

    // FIXME to remove this fence, we need to bind stream to the plan which is not done yet
    exec_space.fence();
    auto out_buffer = scoped_plan.template buffer_data<OutRightViewType, OutViewType::rank()>(out_extents);;
    {
      Kokkos::Profiling::ScopedRegion region("exec_plan[TPL_rocFFTMPIExec]");
      auto* in_data  = in.data();
      auto* out_data = out_buffer.data();
      KOKKOSFFT_CHECK_ROCFFT_CALL(rocfft_execute(scoped_plan.plan(direction), (void**)&in_data,
