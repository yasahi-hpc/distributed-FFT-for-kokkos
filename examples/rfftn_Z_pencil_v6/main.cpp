...
void distributed_fft(int px, int py);
...
void distributed_fft(int px, int py) {
  ...
  KOKKOSFFT_THROW_IF((px * py) != nprocs, "nprocs must be equal to (px, py)");
  ...
  if (rank == 0) {
    std::cout << "Distributed Z-pencil rFFT v6 completed successfully!"
              << "It took: " << seconds << " [s]" << std::endl;
  }
...
int main(int argc, char** argv) {
  ...
  int px      = std::stoi(IO::get_arg(kwargs, "px", "2"));
  int py      = std::stoi(IO::get_arg(kwargs, "py", "2"));

  Kokkos::initialize(argc, argv);
  { distributed_fft(px, py); }
  Kokkos::finalize();
  ::MPI_Finalize();
