...
void initialize(const int rank, const int nprocs, RealView1DType& x,
  ...
  const int nx_local0 = u.extent(2), ny = u.extent(1), nz = u.extent(0);
  const int nx        = x.extent(0);
  const int nx_local1 = ikx.extent(0);
  ...
  for (int jz = 0; jz < nz; jz++) {
    for (int jy = 0; jy < ny; jy++) {
      for (int jx = 0; jx < nx_local0; jx++) {
        int gjx         = jx + rank * nx_local0;
...
void compute_derivative(const std::size_t nx, const std::size_t ny,
  ...
  const int nx_buf1 = ((nx / 2 + 1) - 1) / nprocs + 1;

  // Data in Y-slab layout
  RealView3D u("u", nz, ny, nx_buf0), dudxy("dudxy", nz, ny, nx_buf0);

  // Data in X-slab layout
  RealView3D u_trans("u_trans", nz, ny_buf, nx);
  ComplexView3D Xslab("Xslab", nz, ny_buf, nx / 2 + 1);

  // Data in Y-slab layout
  ComplexView3D Yslab("Yslab", nz, nx_buf1, ny);
  ComplexView2D ikx("ikx", nx_buf1, ny), iky("iky", nx_buf1, ny);

  // Buffers used for MPI communications
  RealView4D send_buffer_0("send_buffer_0", nprocs, nz, ny_buf, nx_buf0);
  RealView4D recv_buffer_0("recv_buffer_0", nprocs, nz, ny_buf, nx_buf0);
  ComplexView4D send_buffer("send_buffer", nprocs, nz, ny_buf, nx_buf1);
  ComplexView4D recv_buffer("recv_buffer", nprocs, nz, ny_buf, nx_buf1);
