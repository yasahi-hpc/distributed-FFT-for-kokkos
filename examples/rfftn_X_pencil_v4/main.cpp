...
void distributed_fft() {
  ...
  ComplexView5D send_x2y("send_x2y", KokkosFFT::Impl::create_layout<LayoutType>(
                                         x2y_buffer_extents));
  ComplexView5D recv_x2y("recv_x2y", KokkosFFT::Impl::create_layout<LayoutType>(
                                         x2y_buffer_extents));
  auto out_y_extents = get_next_extents(gout_shape, mid_topology, mid_map);
  ComplexView4D Ypencil(
      "Ypencil", KokkosFFT::Impl::create_layout<LayoutType>(out_y_extents));
  ...
  ComplexView5D send_y2z("send_y2z", KokkosFFT::Impl::create_layout<LayoutType>(
                                         y2z_buffer_extents));
  ComplexView5D recv_y2z("recv_y2z", KokkosFFT::Impl::create_layout<LayoutType>(
                                         y2z_buffer_extents));
  auto out_z_extents = get_next_extents(gout_shape, out_topology, out_map);
  ComplexView4D Zpencil(
      "Zpencil", KokkosFFT::Impl::create_layout<LayoutType>(out_z_extents));
