...
void distributed_fft() {
  ...
  // 2) Local block sizes for XY-slab
  std::size_t nbatch = 5;
  ...
  map_type in_topology  = {1, 1, nprocs, 1};  // XY-slab
  map_type out_topology = {1, nprocs, 1, 1};  // XZ-slab
  map_type src_map      = {0, 1, 2, 3};

  // Start: XY-slab, End: XZ-slab
  ...
  // Do you local 2D FFTs along XY:
  KokkosFFT::rfft2(exec, in, out_x, KokkosFFT::Normalization::backward, KokkosFFT::axis_type<2>{1, 0});

  Block block_xy2xz(exec, out_x, XZ_slab, send_xy2xz, recv_xy2xz, src_map, 0,
                    dst_map, 1, MPI_COMM_WORLD);
  block_xy2xz(out_x, XZ_slab);

  // Do you local 1D FFTs along Z:
  KokkosFFT::fft(exec, XZ_slab, XZ_slab, KokkosFFT::Normalization::backward, -1);

  // XY-slab to XZ-slab transpose + local 1D FFT along Z
  /*
  ...
  map_type out_map = get_dst_map<Kokkos::LayoutRight, 4>(src_map, out_axis01);
  ...
  // Allocate XZ-slab extents
  auto xz_extents = get_next_extents(gout_shape, out_topology, mid_map);
