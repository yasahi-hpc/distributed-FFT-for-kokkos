...
void distributed_fft() {
  ...
  Kokkos::parallel_for(
      "pack-x2y", policy_x2y, KOKKOS_LAMBDA(int ix, int iy, int iz, int ib) {
        for (int p = 0; p < Px; p++) {
          ...
          send_x2y(p, ix, iy, iz, ib) = gix < nxh
                                            ? out(gix, iy, iz, ib)
  ...
  Kokkos::parallel_for(
      "unpack-x2y", policy_x2y, KOKKOS_LAMBDA(int ix, int iy, int iz, int ib) {
        for (int p = 0; p < Px; p++) {
          ...
          in_Ypencil(ix, iz, ib, giy) = recv_x2y(p, ix, iy, iz, ib);
        }
      });
  ...
  Kokkos::parallel_for(
      "pack-y2z", policy_y2z, KOKKOS_LAMBDA(int ix, int iy, int iz, int ib) {
        for (int p = 0; p < Py; p++) {
          int giy                     = iy + p * ny_local;
          send_y2z(p, ix, iy, iz, ib) = in_Ypencil(ix, iz, ib, giy);
        }
      });
  ...
  Kokkos::parallel_for(
      "unpack-y2x", policy_y2z, KOKKOS_LAMBDA(int ix, int iy, int iz, int ib) {
        for (int p = 0; p < Py; p++) {
  ...
  Kokkos::parallel_for(
      "unpack-z2y", policy_z2y, KOKKOS_LAMBDA(int ix, int iy, int iz, int ib) {
        for (int p = 0; p < Py; p++) {
          int giy                      = iy + p * ny_local;
          in_Ypencil2(ix, iz, ib, giy) = recv_z2y(p, ix, iy, iz, ib);
        }
      });
  ...
  policy_type policy_y2x(exec, {0, 0, 0, 0},
                         {nx_local, ny_local, nz_local, nbatch}, {4, 4, 4, 4});
  Kokkos::parallel_for(
      "pack-y2x", policy_y2x, KOKKOS_LAMBDA(int ix, int iy, int iz, int ib) {
        for (int p = 0; p < Px; p++) {
          int giy                     = iy + p * ny_local;
          send_y2x(p, ix, iy, iz, ib) = in_Ypencil2(ix, iz, ib, giy);
        }
      });
  ...
  Kokkos::parallel_for(
      "unpack-y2x", policy_y2x, KOKKOS_LAMBDA(int ix, int iy, int iz, int ib) {
        for (int p = 0; p < Px; p++) {
          int gix = ix + p * nx_local;
          if (gix < nxh) {
            out(gix, iy, iz, ib) = recv_y2x(p, ix, iy, iz, ib);
          }
        }
      });
