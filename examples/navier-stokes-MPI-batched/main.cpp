...
struct Grid {
  ...
  Grid(int rank, int px, int py, int nx, int ny, int nz, double lx, double ly,
    ...
    // Wavenumbers
    extents_type gout_extents{DIM, std::size_t(nx), std::size_t(ny),
                              std::size_t(nz / 2 + 1)};
    ...
    for (int ikz = 0; ikz < h_ksq.extent_int(2); ikz++) {
      for (int iky = 0; iky < h_ksq.extent_int(1); iky++) {
        for (int ikx = 0; ikx < h_ksq.extent_int(0); ikx++) {
          int gikx             = ikx + out_starts.at(1);  // Global index in x
    ...
    auto sub_gkx = Kokkos::subview(
        gkx, Kokkos::make_pair(out_starts.at(1),
                               out_starts.at(1) + m_kx.extent_int(0)));
    auto sub_gky = Kokkos::subview(
        gky, Kokkos::make_pair(out_starts.at(2),
                               out_starts.at(2) + m_ky.extent_int(0)));
    auto sub_gkzh = Kokkos::subview(
        gkzh, Kokkos::make_pair(out_starts.at(3),
                                out_starts.at(3) + m_kzh.extent_int(0)));
...
struct Variables {
  ...
  Variables(const Grid& grid, bool suppress_diag, double v0 = 1.0) {
    ...
    extents_type gin_extents{DIM, nx, ny, nz},
        gout_extents{DIM, nx, ny, nz / 2 + 1};
    ...
    auto [nin0, nin1, nin2, nin3]     = in_extents;
    auto [nout0, nout1, nout2, nout3] = out_extents;
    ...
    for (std::size_t iz = 0; iz < nin3; iz++) {
      for (std::size_t iy = 0; iy < nin2; iy++) {
        for (std::size_t ix = 0; ix < nin1; ix++) {
          std::size_t gix = ix + in_starts.at(1);  // Global index in x
          std::size_t giy = iy + in_starts.at(2);  // Global index in y
          std::size_t giz = iz + in_starts.at(3);  // Global index in z
    ...
    if (!suppress_diag) {
      ...
      KokkosFFT::Distributed::Plan plan(
          exec, m_u, m_uk, KokkosFFT::axis_type<3>{1, 2, 3}, grid.m_in_topology,
...
class NavierStokes {
  ...
  using DistributedPlanType =
      KokkosFFT::Distributed::Plan<execution_space, View4D<double>,
