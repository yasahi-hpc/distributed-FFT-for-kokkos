...
def initialize(nx: int, ny: int, nz: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray,
    ...
    ikx = np.zeros((nx//2+1, ny), dtype=np.complex128)
    iky = np.zeros((nx//2+1, ny), dtype=np.complex128)

    for iy in range(ny):
        for ix in range(nx//2):
            ikx[ix, iy] = 1j * 2 * np.pi / lx * ix

    for iy in range(ny):
        for ix in range(nx//2+1):
            _iy = iy if iy < ny//2 else iy - ny
            iky[ix, iy] = 1j * 2 * np.pi / ly * _iy
...
def compute_derivative(nx: int, ny: int, nz: int) -> np.float64:
    ...
    # (nz, ny, nz) -> (nz, ny, nz/2+1)
    ux_hat = np.fft.rfft(u, axis=-1)
    
    # Transpose into y-pencil layout
    # (nz, ny, nz/2+1) -> (nz, nz/2+1, ny)
    uxy_hat = np.transpose(ux_hat, axes=(0, 2, 1))
    
    # Forward transform u -> u_hat (=FFT (u))
    # (nz, nz/2+1, ny) -> (nz, nz/2+1, ny)
    uxy_hat = np.fft.fft(uxy_hat, axis=-1)

    # Compute derivatives by multiplications in Fourier space
    uxy_hat = ikx * uxy_hat + iky * uxy_hat

    # Backward transform u_hat -> u (=IFFT (u_hat))
    # (nz, nz/2+1, ny) -> (nz, nz/2+1, ny)
    uxy_hat = np.fft.ifft(uxy_hat, axis=-1)
    
    # Transpose into x-pencil layout
    # (nz, nz/2+1, ny) -> (nz, ny, nz/2+1)
    ux_hat = np.transpose(uxy_hat, axes=(0, 2, 1))
    
    # Backward transform u_hat -> u (=IFFT (u_hat))
    # (nz, ny, nz/2+1) -> (nz, ny, nz)
    u = np.fft.irfft(ux_hat, axis=-1)
